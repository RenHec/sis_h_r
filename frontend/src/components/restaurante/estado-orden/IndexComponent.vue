<template>
  <v-container fluid>
    ...
    <template v-slot:item.inicia="{ item }">
      <span>{{ getTextTitle(item.inicia) }}</span>
    </template>
    <template v-slot:item.finaliza="{ item }">
      <span>{{ getTextTitle(item.finaliza) }}</span>
    </template>
    ...
    <template v-slot:top>
        ...
        <v-toolbar-title>Estado de las órdenes</v-toolbar-title>
        <v-spacer></v-spacer>
...
export default{
  ...
  data(){
    return{
      ...
      headers:[
        ...
        { text: 'Inicia', value: 'inicia' },
        { text: 'Finaliza', value: 'finaliza' },
  ...
  created(){
    events.$on('close_form_new_order_status',this.eventCloseFormNewOrderStatus)
    events.$on('close_form_update_order_status',this.eventCloseFormUpdateOrderStatus)
  },
  beforeDestroy(){
    events.$off('close_form_new_order_status')
    events.$off('close_form_update_order_status')
  },
  watch: {
    options: {
      handler() {
  ...
  methods:{
    ...
    eventCloseFormNewOrderStatus(){
      this.initializeView()
      this.recharge()
    },
    eventCloseFormUpdateOrderStatus(){
      this.initializeView()
      this.recharge()
    },
    updateRecord(item){
